...
4. **接进菜单(必做,不接=孤儿页面)**:先查 `js/layout.js` 的 `MENU_GROUPS` 是否已有该功能的占位项——**有则向占位对齐**(`goMenu` 内 PAGES 映射加一行 key→文件,页面 `activeLeft` 用占位 key,不新造菜单项);确认没有才在 MENU_GROUPS 新增子项。菜单结构仿线上 CCOS,页面向菜单靠,不是菜单向页面靠。
5. **验证**:浏览器打开,确认标题栏、左侧菜单、Tab、功能正常;从首页菜单点进来能到、左侧正确高亮
...
**列表页范式(对齐线上 WinForms 模式,不自创 Web 后台风格)**:
- **列表无操作列**——操作走工具栏按钮(查询/新建/编辑/启用/停用/删除/导出),行点击选中(`row--selected`),工具栏操作作用于选中行,无选中时提示"请先选中一行"
- **分页固定底部**——content 内固定顺序:`查询区 → 工具栏 → .grid-wrap(表格区,flex:1 自滚) → .pager`;禁止在中间插普通 div 中转(会打断 flex 链,分页被顶走)

**弹窗范式(三层结构,防 sticky 表头穿透)**:
- 正确:`.rw-modal`(fixed + z-index:2000 外壳) 包 `.rw-modal-mask`(蒙层) + `.rw-modal-panel`(面板)
- 错误:直接拿 `.rw-modal-mask`(absolute 无 z-index)当顶层容器——会被 sticky 表头(z-index:1)遮挡穿透
- **内容会超高的弹窗**(行数多的清单/表格):给面板加 `.rw-modal-panel--scroll` 修饰类(面板限高 `100vh-64px`、body 内滚、header/footer 固定)。**含浮层控件的弹窗别加**——body 一旦可滚,其内部绝对定位浮层(值多选下拉等)会被裁剪,只能改用分页/限高清单区等替代

### 客户端能力边界(设计交互前先过一遍)

原型是 HTML 画的,但**落地形态是 C#/.NET8 + WinForms 桌面客户端**。Web 上顺手的手法客户端要么做不了、要么自绘成本极高,**别把 Web 特有交互画进 PC 端原型**:

| 别用(Web 特有) | 改用(客户端范式) |
|---|---|
| hover 浮层/气泡卡片(尤其承载表格、按钮等富内容) | 常显列、页签、弹窗;轻提示用 ToolTip |
| 滑出抽屉、过渡/淡入动画 | 模态对话框(ShowDialog) |
| 半透明蒙层、圆角阴影卡片风 | 系统控件观感,视觉对齐线上 WinForms |
| 响应式重排/流式布局 | 固定栅格(视口 1280px+),Anchor/Dock 式布局思路 |
| 容器内独立滚动(外层页面滚 + 内层表格再滚) | 一个窗口只保留一层主滚动 |
| 浏览器式路由 / "返回上一页" | 打开新窗体(模态/非模态);多级返回要自己维护 |
| chips/标签溢出"+N 项"、自定义控件外观 | 用现成控件组合;确需自绘的先确认开发成本 |
| 无限滚动、骨架屏 | 分页(见上方分页范式) |

客户端反而擅长、可放心用的:**Tab 页签、模态对话框、工具栏按钮 + 行选中、DataGridView(排序/拖列宽/冻结列)、右键菜单、快捷键、本地文件导出与打印、多窗口并存(可同时开多个详情窗对比)**。

**拿不准的交互,先查 `code/pc/`(WinForms 生产代码)有没有先例**——有先例再用;原型里的演示性效果要与真实可实现的交互区分清楚。
